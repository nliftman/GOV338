...
#load cosponsorship
cosponsorship <- as.matrix(import("/Users/naomiliftman/Desktop/GOV338/Final Project/Final Datasets/first models/weighted_112.csv")) 
```

# Three dimensional matrix 112th
```{r}
#stack em baby
state_matrices <- array(NA, c(7, length(catholic[1,]),length(catholic[1,]))) 
...
religion_112$names <- c("Intercept", "Catholic", "Jewish", "Other", "Protestant", "Unknown", "Party", "Ideology")
...
#load cosponsorship
cosponsorship <- as.matrix(import("/Users/naomiliftman/Desktop/GOV338/Final Project/Final Datasets/first models/weighted_117.csv"))
...
#need to make sure you label in the order you read them into R
religion_117$names <- c("Intercept", "Catholic", "Jewish", "Other", "Protestant", "Unknown", "Party", "Ideology")
...
# Find SE?
```{r}
td_coef <- function (religion_117) {
  sigma2 <- sum(religion_117$residuals ^ 2) / religion_117$df.residual
  Rinv <- backsolve(religion_117$qr$qr, diag(religion_117$rank))
  sqrt(rowSums(Rinv ^ 2) * sigma2)
  }

td_coef(religion_117)
```
