...
# im actually the stupidest man alive
```{r}
#making an i and j 
relig_117_fixed <- relig_117_fixed %>% 
  mutate(member_i=name) %>% 
  rename(member_j = name) %>% 
  mutate(religion_i = religion) %>% 
  rename(religion_j = religion)

#creating all the possible combinations
combos <- relig_117_fixed |>
  expand(member_i, member_j) |>
  filter(member_i != member_j) 

#merge back in religion
combos_rel <- combos %>% 
  right_join(relig_117_fixed, by = c("member_i" = "member_i")) %>% 
  select(member_i, member_j.x, religion_i)

#merge religion of j
combos_rel <- combos_rel %>% 
  right_join(relig_117_fixed, by = c("member_j.x" = "member_j")) 
...
#export it for easier work!
write.csv(d_112, "religion_117_but_like_ithinkitworksnow.csv", row.names=FALSE)
```
